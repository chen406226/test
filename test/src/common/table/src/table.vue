<template>
  <div class="el-table"
    ...
    <div
        ...
        <div :style="{height: `${allData.length * 48}px`}">
        <div :style="{transform: `translateY(${virTop}px)`}">
...
export default {
  ...
  methods: {
    ...
    doComputedVir: debounce(100, false, function(t) {
      let it = Math.floor(t/50)
      let yu = t % 50
      let mt = Math.round(this.maxHeight/50)
      if (t>this.beforeVirTop) {
        if (it - this.virStartIndex > mt) {
          this.virStartIndex = Math.floor(t/50)
          this.virTop = t 
          this.setVirData()

          // console.log(it,mt,this.beforeVirTop,this.virTop,this.virStartIndex)
          // this.bodyHeight.scrollTop = this.virStartIndex * 48
        }
      } else if (it <= this.virStartIndex -1) {
        this.virStartIndex = it - mt
        this.virTop = t - this.maxHeight
        if (this.virStartIndex < 0) {
          this.virStartIndex = 0
        }
        if (t < this.maxHeight) {
          this.virStartIndex = 0
        }
      ...
      console.log(t,'tttttt',it,this.virStartIndex,this.beforeVirTop,this.bodyHeight)
      this.beforeVirTop = t
    }),
    setVirData () {
      console.log(this.allData.length,'lkjsdflkjsldkfjlksjdflksjdflkjl')
      let it = Math.round(this.maxHeight/50)

      this.$emit('setVirData', this.allData.slice(this.virStartIndex,this.virStartIndex+it * 3))
    },
    // TODO 使用 CSS transform
    syncPostion: throttle(20, function() {
      console.log('KKJLKJLJLJLKLK')
