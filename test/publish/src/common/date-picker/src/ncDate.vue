<template>
  <div
      v-show="visible"
      class="el-date-picker el-popper ncDate"
      :class="[{
      ...
      <div class="el-picker-panel__body">
        ...
        <div
          ...
          <span
              ...
              :class="{ active: currentView === 'month' }">{{(month+1).toString().padStart(2,'0')}}</span>
          <span style="font-size: 11px;margin-left:30px;" v-if="showGeneralization">{{!(abnormalAttendance.length>0 || normalAttendance.length>0) ? '当月无考勤记录' : abnormalAttendance.length ? `当月考勤异常${abnormalAttendance.length}天` : '当月考勤正常'}}</span>
          <button
              type="button"
              @click="nextYear"
              :aria-label="t(`el.datepicker.nextYear`)"
              class="el-picker-panel__icon-btn el-date-picker__next-btn el-icon-caret-right">
          </button>
          <button
              type="button"
              @click="nextMonth"
          ...
          <button
              ...
              class="el-picker-panel__icon-btn el-date-picker__prev-btn el-icon-caret-left">
          </button>
        </div>
        ...
        <div class="el-picker-panel__content">
          <date-table
              ...
              :value="value"
              :format="dateFormat"
              :normalAttendance="normalAttendance"
              :abnormalAttendance="abnormalAttendance"
...
export default {
  ...
  methods: {
    ...
    prevMonth () {
      ...
      this.$emit('switchDate', this.date)
    },
    ...
    handleDatePick (value) {
      ...
      if (this.month != value.getMonth()) {
        this.$emit('switchDate', value)
      }
...
<style lang="css">
.ncDate .el-picker-panel__body .el-date-picker__prev-btn {
  float: right;
}
.ncDate .el-picker-panel__body .el-picker-panel__content {
  width: 370px;
  margin-top: 0;
  margin-bottom: 0;
}
.ncDate .el-ck-picker-header {
  text-align: left;
  margin: 0 12px;
  width: 370px;
}
